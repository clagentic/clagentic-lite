...
| `.claude-plugin/marketplace.json` | plugin marketplace manifest — declares the `clagentic-lite` plugin (checked-in source; from lr-1b5a31 the actual install marketplace source is the RENDERED copy, see below — this file documents the checked-in agent/skill tree that render reads from) |
...
| `plugins/clagentic-lite/agents/{builder,reviewer,auditor,merge-gate,troubleshooter}.md` | role contracts — READ-ONLY INPUT to `_render_clagentic_lite_plugin_dir` (`bin/clagentic-lite`, lr-1b5a31). Never installed directly; always rendered first into `$CLAGENTIC_LITE_HOME/.clagentic/rendered-plugin/` (config-aware — carries `model: role:<role>-chain` on reviewer/auditor/merge-gate when router injection is active) and that rendered copy is what `claude plugin install` actually installs. One plugin, always — see "Single-plugin config-aware render" below. |
...
The `clagentic-lite` plugin (agents + skills, installed globally via `claude plugin install`) is a **fifth** generated artifact, with its own mechanism — not the version-constant table above, because its output depends on *live config*, not just template content.

**One plugin, always.** `clagentic-lite init`/`update` never install `plugins/clagentic-lite/` directly. They always call `_render_clagentic_lite_plugin_dir` (`bin/clagentic-lite`) first, which reads the checked-in agent files as read-only input and writes a rendered copy to `$CLAGENTIC_LITE_HOME/.clagentic/rendered-plugin/` — injecting `model: role:<role>-chain` into reviewer/auditor/merge-gate frontmatter when router injection is active (`CLAGENTIC_ROUTER_URL` + `CLAGENTIC_ROUTER_INJECT_AGENT_MODEL=1`, and the role's own `CLAGENTIC_<ROLE>_CMD` is non-`claude`), omitting it otherwise. That rendered directory — never the checked-in tree — is what gets registered as a marketplace source and installed. There is exactly one plugin named `clagentic-lite` at any time; a second, differently-named plugin overlay (the pre-lr-1b5a31 `clagentic-lite-router` design) cannot exist by construction, so the same-agent-name dispatch collision that motivated this design cannot recur for this plugin. `clagentic-lite doctor`'s plugin-collision check (`_doctor_check_plugin_collision`) remains as a generic backstop for a *third-party* plugin registering a colliding agent name — not specific to this one.

**Render stamp — the class mechanism.** `PLUGIN_RENDER_VERSION` (bump on any change to the render LOGIC itself — the injection point, the triple list, the plugin.json shape) is combined with `_render_config_fingerprint` (every live config knob the render function actually branches on) into a single stamp string (`_render_stamp`), written into the rendered `plugin.json`'s `clagenticStamp` field. **Rule: any new config knob added to `_render_clagentic_lite_plugin_dir`'s branching logic must also be added to `_render_config_fingerprint`.** Without that, a config change that legitimately changes render output goes undetected by the stamp comparison — exactly the generality the mechanism exists to guarantee. `clagentic-lite doctor`'s `_doctor_check_render_stamp_staleness` recomputes the stamp fresh against current config on every run and compares it to what is actually installed, advising `clagentic-lite update` on mismatch. `update` itself always re-renders and re-installs unconditionally (cheap, idempotent) rather than gating on a stamp comparison first — the stamp exists for *doctor's visibility*, not to decide whether update runs.

**Migration.** `_migrate_legacy_router_plugin` runs unconditionally at the top of `_install_clagentic_lite_plugin` (both `cmd_init` and `cmd_update`), regardless of current router config, and uninstalls a pre-lr-1b5a31 `clagentic-lite-router` plugin install if found. One `clagentic-lite update` run converges any machine to the single-plugin model, whichever direction its config previously drifted.
