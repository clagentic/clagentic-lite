...
| `host_adapter_open_change_request BASE HEAD [BODY_FILE]` | Open (or find and reuse) a PR/MR for `HEAD` against `BASE`. This is the refactored seam `cmd_ship`'s PR-open step now calls — before this task it called `gh pr view`/`gh pr create` directly. `BODY_FILE` (lr-429b32) is optional: when given, its contents become the PR body on CREATE only — an already-open PR is reused as-is, body unchanged. The caller renders `BODY_FILE` (see "Ship-time PR body" below); this function only transports it. |
...
**Repo-state scoping (INV-6, code-review follow-up on PR #163).** Every `git -C ...` call in `scripts/host-adapter.sh` that reads repo state (the remote-URL sniff, the current-branch reads in the `gh` adapter's comment paths) is gated on `_host_adapter_repo_root_is_scoped` first — a local mirror of `_git_repo_root_is_scoped` (see "Review ledger and anchored verdicts" above for that predicate's own rationale). Without this, `REPO_ROOT` being a subdirectory of an unrelated ANCESTOR git repo (a valid wrapper layout) makes `git -C "$REPO_ROOT" ...` silently walk up and resolve the ancestor's `.git` instead — `host_adapter_available` could report success against the wrong repository's remote, and the publish path could post a review verdict's findings to the WRONG repository's change-request thread. `scripts/test_host_adapter_publish.py`'s `TestHostAdapterRepoScopingSweep` both reproduces the exact ancestor-repo scenario directly and sweeps the file for any future unguarded `git -C` call.
...
**Section 2 (review provenance) is the only section backed by real data, and it is reused, not re-derived.** `_build_ship_pr_body` looks up the SAME anchored-verdict data `cmd_merge_gate` and `_publish_review_verdict` already consult — `ledger_latest_for_branch` against `_review_ledger_path`/the current branch — and, when the latest ledger entry's `head_sha` matches the PR's own head exactly, renders it through `_render_review_verdict_lines`, the shared core `_build_review_verdict_comment_body` (the posted review-verdict comment) also calls. One rendering function, two callers, per AGENTS.md's reuse-first discipline — the PR body's review-provenance section and the posted comment can never drift out of sync in what they consider "the" verdict for a given head.

**Degrade honestly — the acceptance bar, not a nicety.** Three distinct review-provenance states, each rendered as an explicit sentence, never a bare heading:

- **Anchored verdict at this exact head_sha exists** — full verdict, `head_sha`, per-severity findings count, and any recurring-finding markers, identical in substance to what the posted comment already carries for the same round.
- **A ledger entry exists for this branch, but at a different head_sha** — rendered as `reviewer: prior verdict recorded (head ..., but it does not cover this PR's head (...) -- treat review as not yet run for this head.` This is the stale-relative-to-what's-shipping case; the section explicitly refuses to imply the older verdict still applies.
- **No usable ledger entry at all** (never reviewed, no `jq`/`python3` available to read the ledger, or the ledger file itself is absent) — rendered as `reviewer: none -- no recorded review verdict for this branch. Run \`clagentic-lite gates review\` ... if cross-vendor review is expected.` This is lr-964f7f's motivating failure mode inverted: that bug asserted a review posture ("cross-vendor review is disabled") contradicting what had actually run; this section is built to never assert a posture the tool cannot back with a recorded verdict on either side — neither a false "clean" nor a false "disabled."

**Sections 1/3/4 have no mechanical source and say so.** `_build_ship_pr_body` has no commit-log or diff-summarizer to draw "what changed and why," "trade-offs," or "out of scope" from — adding one is out of this task's scope (and would duplicate work AGENTS.md's non-goals list already reserves against). Each of these three sections renders an explicit placeholder sentence naming the gap and directing the author to fill it in by hand, never a fabricated summary and never an empty heading with nothing under it — the same "never a confident-looking empty heading" rule the review-provenance section follows for its own no-verdict case.
...
and both call sites reuse it. This repo's own `.clagentic/semgrep-exclude`
currently carries no active exclusions (a prior committed exclusion whose
stated rationale did not correspond to any code actually present in this
repo was removed during PR #159 review — see that file's own header
comment); the file stays committed as a live demonstration of the format.
...
preferences are a per-repo opt-in, never a value hardcoded into `gates.sh`
itself. With no exclude files and no `CLAGENTIC_SEMGREP_CONFIG` set,
`cmd_sast`'s semgrep argv is byte-identical to the pre-lr-321e18 invocation
— this is the load-bearing no-regression property the ladder and the config
override are both built around.

**A pinned config is never silent either.** A pinned config can replace
`--config=auto` with a policy path that disables every rule — the same
silent-suppression exposure the visibility mechanism above closes for the
exclude ladder, applied to the config pin (security review, PR #159). When
`CLAGENTIC_SEMGREP_CONFIG` is active, `cmd_sast` echoes the pinned path to
stderr (`[gates/sast] using pinned config: <path>`) and appends `; config=
<path>` to the `gate_runs.details` string on both the baseline and
full-tree pass branches, alongside the exclusion count/id list when both
are active. `_sast_pinned_config_from_argv` (`scripts/gates.sh`) detects
the pin from `cmd_sast`'s own reconstructed argv.
...
**The suppression-review loop, and why it is not aspirational.** lr-321e18 shipped `.clagentic/semgrep-exclude` with **zero active entries**. A `python.sqlalchemy.security.sqlalchemy-execute-raw-query` exclusion was proposed with a plausible-sounding justification (injection-safe sqlalchemy/psycopg2/`sql.Identifier` usage); a security review (PR #159) checked the claim against the actual codebase and found no sqlalchemy, psycopg2, or `sql.Identifier` code anywhere in this repo, and the two engram IDs cited alongside the justification did not resolve in the referenced memory store. The entry was removed rather than shipped — see `.clagentic/semgrep-exclude`'s own header comment for the retained record. This is the policy working as intended, not an edge case: a suppression's justification is checked against the repo it claims to describe, and an unverifiable justification does not ship, regardless of how reasonable it reads in isolation. That review's rulebook addition below (`clagentic-lite.new-suppression-review`) makes this the standing expectation for every future suppression, not a one-off catch.
...
**Deterministic-gates payload block (lr-367a21) — informational only, no new gate.** `build_gate_summary`'s `deterministic_gates` field (see the field list above) lets the Merge Gate prompt describe what the payload actually contains instead of misstating it — it does not add a check, a blocking behavior, or a bypass. `cmd_secrets`/`cmd_deps`/`cmd_sast` already ran and already fail-closed, upstream of the Merge Gate, before this field is ever read; `_read_deterministic_gates` (`scripts/gates.sh`) only reads back the LATEST `gate_runs` row each of those gates already logged via `cmd_log_run` — one `id` lookup plus one `outcome` query plus one `details` query per gate (`secrets`/`deps`/`sast`), each returning a single column, so an embedded newline in `details` can never bleed into `outcome` (lr-acf632; the prior shape read both columns in one delimited row via `cut`, which is line-oriented and mis-split on a multi-line `details` value) — preserving the distinct `pass`/`warn`/`skip`/`block` outcomes and a `null` for "no logged row at all." The read follows the same fail-open, degrade-never-block pattern as the per-step-failure hint read in Gate 3 ([-f "$ADB"] && command -v sqlite3) and `platform.sh`'s `ds_audit_log`/`ds_sqlite3` (audit-db access is best-effort by contract everywhere else in this codebase): no `sqlite3`, no `audit.db`, or an unreadable/corrupt DB all degrade to every gate field `null` plus `audit_db_unavailable: true`, and `build_gate_summary` still returns its normal payload — the Merge Gate still proceeds to the LLM call exactly as it does today when this block is absent entirely. Mirrored identically in both JSON-emitter branches of `build_gate_summary` (jq and python3) so they cannot disagree — the python3 branch receives the already-computed JSON object as an argument rather than re-querying `audit.db` a second time. Each gate's `details` text is routed through `_llm_field_sanitize` (the same sanitizer named in "Round-trip sanitization" below — control-byte strip, fence-label defang, length cap) at the single shared read point inside `_read_deterministic_gates`, before either emitter branch ever sees it — `details` is attacker-reachable free text (e.g. `.clagentic/semgrep-exclude` rule-id lines flow into a `sast` `pass` details string), the same round-trip shape as adversarial findings and the invariant feed. `outcome` is a closed `pass`/`warn`/`skip`/`block` set, never sanitized, so the outcome distinction this block exists to preserve is untouched by the sanitize step.

**Fenced, not just sanitized (lr-92d931).** Sanitizing `details` closes delimiter-forgery/control-byte injection, but `deterministic_gates` itself was, until this fix, the one field in this payload that was sanitized without also being fenced — a divergence from the "every external-text payload field reaching an LLM prompt is both sanitized and fenced" convention (see "CONVENTION" under Gate 6 below) that `adversarial_findings_fenced` and the invariant feed already followed. `build_gate_summary` now additionally emits `deterministic_gates_fenced`, a text rendering of the same (already-sanitized) `deterministic_gates` object delimited by `===BEGIN DETERMINISTIC GATES DATA===` / `===END DETERMINISTIC GATES DATA===` — produced by `_fence_deterministic_gates` (`scripts/gates.sh`), which mirrors `_fence_adversarial_findings`'s own shape and, unlike that function, is called identically from BOTH `build_gate_summary` emitter branches (not reimplemented per-branch) so the two cannot diverge on the fence text itself. `_llm_field_sanitize`'s defang list now also covers this label, so a hostile `details` string containing the literal fence marker cannot forge a fake end-of-block boundary once rendered. `ds_merge_gate_prompt` (`scripts/llm-client.sh`) instructs the Merge Gate to treat `deterministic_gates_fenced` as data, not instructions, with the same framing `adversarial_findings_fenced` gets — this does not change the informational-only posture above: neither `deterministic_gates` nor its fenced rendering can gate a merge-gate decision on its own.
...
**CONVENTION: every external-text payload field reaching an LLM prompt is BOTH sanitized AND fenced (lr-92d931, settling a question lr-367a21 left open).** The rule is deliberately broad, not "only fence fields the model reasons over": the narrower alternative requires every future author to correctly classify, under time pressure while adding an unrelated feature, whether their new field is one the model reasons over — and that is exactly the judgment call that failed for `deterministic_gates.details` (see below): sanitized at lr-367a21, explicitly documented as informational-only, and still left unfenced. "Informational-only" is a property of the prompt's CURRENT instruction text, not of the field itself, and is one edit away from changing — a fencing decision that silently depends on prompt wording staying put is not a durable convention. The cost asymmetry also settles it: fencing a field the model never reasons over costs a few bytes of prompt; not fencing a field the model turns out to reason over is a prompt-injection surface. `_llm_field_sanitize` (`scripts/platform.sh`) is the sole sanitizer for this class of round-trip and documents this convention at its own definition; `_fence_adversarial_findings` and `_fence_deterministic_gates` (both `scripts/gates.sh`) are the fence-rendering helpers for their respective fields — sanitize and fence remain two separate calls a payload-building caller must invoke in sequence (not a single combined helper: sanitization happens once, at each field's own write/read boundary, independent of whether or how many times that field is later fenced for a given prompt, so collapsing the two into one call would either re-sanitize on every render or force fencing to happen at the wrong layer). When you add a new external-text field to any prompt payload built in `gates.sh`/`llm-client.sh`: sanitize it through `_llm_field_sanitize`, add its own fence label to that function's defang list, and wrap it in that fence at the payload-construction site — shipping one without the other reintroduces this exact gap.

**Class sweep, recorded (lr-92d931).** Every external-text field reaching `ds_merge_gate_prompt`, `ds_review_prompt`, or `ds_adversarial_prompt` (`scripts/llm-client.sh`), checked against the convention above at the time of this fix:

| Prompt | Field | Sanitized? | Fenced? | Status |
|---|---|---|---|---|
| `ds_review_prompt` | deferrals (`.clagentic/deferrals.json`, six-plus schema fields) | Yes — allowlisted then `_llm_field_sanitize`'d per field | Yes — `===BEGIN/END DEFERRED FINDINGS DATA===` | Already both (pre-existing, lr-4f8316) |
| `ds_review_prompt` | change-class hint (commit-message trailer) | Yes — `_llm_field_sanitize` | Yes — `===BEGIN/END CHANGE-CLASS HINT DATA===` | Already both (pre-existing, lr-4f8316) |
| `ds_adversarial_prompt` | invariants (`invariants.json`, sanitized at write time) | Yes — `_llm_field_sanitize` via `_invariant_feed_append` | Yes — `===BEGIN/END INVARIANTS DATA===` | Already both (pre-existing, lr-cda4b9) |
| `ds_adversarial_prompt` | change-class hint (commit-message trailer) | Yes — `_llm_field_sanitize` | Yes — `===BEGIN/END CHANGE-CLASS HINT DATA===` | Already both (pre-existing, lr-4f8316) |
| `ds_merge_gate_prompt` (payload) | `adversarial_findings` / `adversarial_findings_fenced` (finding file/category/message) | Yes — `_llm_field_sanitize` via `_sanitize_adversarial_findings_json` | Yes — `===BEGIN/END ADVERSARIAL FINDINGS DATA===` | Already both (pre-existing, lr-e2b975) |
...
| `ds_merge_gate_prompt` (payload) | `review` (raw `last-review.json` object) | No | No | Deliberately out of scope — see "Premise re-check flagged, not fixed here" below; different mechanism, larger blast radius, tracked separately as lr-793e91 |
| `ds_merge_gate_prompt` (payload) | `adversarial` (raw markdown prose) | No | No | Same as `review` above — reaches the Merge Gate as fallback source-of-truth only when `adversarial_findings` is empty/absent; tracked under lr-793e91 |
...
**Variable-assembled details strings — runtime-checked helper, not a smarter static matcher (lr-2e8444).** The lint's regex only ever sees a literal double-quoted details string in gates.sh's own source text. A `cmd_log_run <gate> pass "$SOME_VAR"` call (bare variable — e.g. cmd_sast's pass sites before this fix) or `cmd_log_run <gate> pass "literal ($SOME_VAR)"` call (mixed literal+variable — cmd_bleed's four `$_BLEED_SCOPE_REASON` sites, cmd_merge_gate's two class/state-suffix sites) is invisible to the static regex in whole or in part. A mixed site is the more dangerous of the two: the literal half is checked and reports clean, producing a positive lint result, while the interpolated half — exactly where a suppression reason like "scope reduced" or "config replaced" lives — is never examined.

This is closed from the runtime side, not by teaching the static matcher to resolve shell variables (open-ended, and cannot see a value that only exists after interpolation at call time regardless): every non-literal `pass` call site in `scripts/gates.sh` routes through `_cmd_log_run_checked_pass GATE DETAILS` instead of calling `cmd_log_run GATE pass DETAILS` directly. That helper checks the fully assembled, post-interpolation details string against the same failure vocabulary at the moment it actually exists, and downgrades the logged outcome from `pass` to `warn` on a hit rather than silently reporting a false-clean pass. A **second static check** inside `cmd_audit_vocab_lint` sweeps for any direct `cmd_log_run <gate> pass ...` call site whose details argument contains a `$` (i.e., is variable-assembled) and flags it as bypassing the checked helper — this is the regression guard that keeps a future non-literal `pass` call site from silently reintroducing the same blindness by simply not routing through `_cmd_log_run_checked_pass`.
