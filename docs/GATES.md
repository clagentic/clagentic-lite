...
**Premise re-check flagged, not fixed here (lr-92d931 sweep).** The `file`/`line`/`category`/`message`/`evidence`/`suggestion` row above waives sanitization on the stated reasoning that "this schema is not currently interpolated into a later prompt." `build_gate_summary`'s `review` field (the raw `last-review.json` object, findings included) is read by the Merge Gate as part of the gate-summary JSON on stdin — the "future change" that reasoning names as the trigger to revisit this already exists today, via a different mechanism (channel: stdin data, not a system-prompt splice) than the invariants/deferrals/deterministic-gates fields this task fenced. Raised as a separate, larger-scope finding rather than folded into this fix — it would mean extending `ds_review_prompt`'s own closed schema and `_sanitize_review_findings_envelope`, not adding a fence label to an existing field. Not filed as a new task within this PR; flagged here for the operator/next task to pick up.
