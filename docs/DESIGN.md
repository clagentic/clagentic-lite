...
| **Reviewer** | `codex` | Read staged diff, return structured findings. Never writes code. | Read, Grep, Glob — no Bash. Enforced via `--allowedTools`/`--disallowedTools` when the resolved chain step is `claude` (see `scripts/llm-client.sh` `invoke_claude`, AGENTS.md Invariants INV-2); NOT independently enforceable against `codex` — the shipped default — which has no confirmed equivalent flag. This is a live, unclosed gap, not merely documented and left inert: every reviewer chain step that resolves to a non-`claude` CLI prints a loud stderr warning on the actual call (`walk_chain`, `scripts/llm-client.sh`) and `clagentic-lite doctor` reports it under "reviewer tool-restriction check" before any review runs. Changing the shipped default away from `codex` was considered and rejected — it would defeat cross-vendor review (§"Cross-vendor is the point" below) to work around a gap in one CLI's flag surface. |
